...
- Sudoers: the powermetrics grant is now pinned to the exact argv the app runs; install-helper.sh refuses to install under a non-root-owned /usr/local. The Jul 13 sudoers file on this Mac still carries the unrestricted grant until `sudo bash zest-smc/install-helper.sh` is re-run (owner action, needs the admin password).
